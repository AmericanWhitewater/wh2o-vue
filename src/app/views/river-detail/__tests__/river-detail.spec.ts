...
import RiverDetail from "@/app/views/river-detail/river-detail.vue";
...
const mockAlerts = [
  {
    ...
    title: null,
    detail:
      "A landowner around the #5 rapid has had some problems with his property being trashed up and property being destroyed (trees cut, fences torn down).  In addition both he and his family have had some nasty encounters with various people (including boaters).  While there is a small parking area near the bridge, it appears that there is no direct public access from the road to the river around the #5 rapid (including around the bridge).  The land owner appears to be simply asking that people respect his property and not to access the river from the road without direct permission!",
    post_date: "2009-07-11 00:00:00",
    revision: 24892,
    post_type: "WARNING",
    user: null,
  },
...
describe("river-detail.vue", () => {
  ...
  it("loads data on initial load", () => {
    ...
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      1,
      "[RIVER_DETAIL] FETCH_RIVER_DETAIL_DATA",
      "123"
    );
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      2,
      "[REACH_GAGES] FETCH_GAGES",
      "123"
    );
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      3,
      "[ALERTS] FETCH_ALERTS_DATA",
      "123"
    );
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      4,
      "[GAGE_METRICS] FETCH_GAGE_METRICS",
      "123"
    );
  });
  ...
  it("displays login prompt / toast when user attempts to edit when not logged in", async () => {
    ...
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      ...
      { kind: "error", title: "Must log in to edit" }
    );
  });
  ...
  it("shows no new notifications icon when reach has alerts", () => {
    ...
    expect(
      wrapper.find(".button-wrapper notificationnew20-stub").exists()
    ).toBe(false);
    expect(wrapper.find(".button-wrapper notification20-stub").exists()).toBe(
      true
    );
  });
  ...
  it("bookmarks reach when not previously bookmarked and notifies user", async () => {
    ...
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      6,
      "[APP_GLOBAL] SEND_TOAST",
      { kind: "success", title: "Bookmark Added" }
    );
  });
  ...
  it("removes bookmarked reach and notifies user", async () => {
    ...
    expect(mockStore.dispatch).toHaveBeenNthCalledWith(
      6,
      "[APP_GLOBAL] SEND_TOAST",
      { kind: "success", title: "Bookmark Removed" }
    );
  });
});
