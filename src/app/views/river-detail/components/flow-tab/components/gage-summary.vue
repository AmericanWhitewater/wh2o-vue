  ...
  <div class="border" @click.stop='select'>
    <div :class="[{'selected':selected},getClass(gage.rc) ]" class="gage-grid">
      ...
      <span class="gage-meta">
        <span v-if="!gage.excluded && gage.delay_update">When updated this gage will wait {{ parseFloat(gage.delay_update/(3600)).toFixed(1) }} hours prior to reporting on the summary page. </span>
        <span v-if="!gage.excluded && !gage.delay_update">When updated this gage will immediately report on the summary page. </span>
        <span v-if="gage.excluded">When updated this gage will <i>not</i> report on the river summary page. </span>
