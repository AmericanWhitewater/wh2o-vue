<template>
  <cv-toolbar id="nwi-map-toolbar">
    ...
    <template v-if="controlEnabled('baseMap')">
      ...
      <cv-dropdown v-model="currentBaseMap" inline label="Basemap">
    ...
    <template v-if="controlEnabled('color')">
      ...
      <cv-dropdown v-model="currentColorBy" inline label="Color by">
    ...
    <template v-if="controlEnabled('fullscreen')">
      <cv-button @click="toggleFullscreen">
        ...
        />
        <Screen20 v-else class="bx--btn__icon bx--toolbar-filter-icon" />
      </cv-button>
    </template>
  </cv-toolbar>
</template>
