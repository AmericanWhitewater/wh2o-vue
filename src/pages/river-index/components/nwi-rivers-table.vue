    ...
    <template v-else-if="reaches && reaches.length > 0">
      <div class="table-container">
        <v-data-table
          ...
          :items="reaches"
          :headers="[
            { text: 'River', value: 'properties.river' },
            { text: 'Class', value: 'properties.class' },
            { text: 'Flow', value: 'properties.gage_0_reading' },
          ]"
        />
      </div>
      <!-- <div
...
export default {
  ...
  methods: {
    handleRowClick(reach) {
      this.$router.push(`/reach-detail/${reach.id}`);
    },
